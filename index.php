    ...
    <div class="container mx-auto flex flex-col md:flex-row items-center justify-between px-6">
        ...
        <div class="mt-6 md:mt-0 md:w-1/2 flex justify-center">
            <div class="relative w-96 h-72 overflow-hidden rounded-lg shadow-lg">
                <img src="uploads/h1.png" alt="Hewan Adopsi" class="w-full h-full object-cover">
            </div>
        </div>
    </div>
</header>
...
<section class="container mx-auto py-16 text-center">
    ...
    <div class="grid md:grid-cols-3 gap-6">
        <?php 
        include 'koneksi.php';
        $query = "SELECT * FROM hewan WHERE status = 'Tersedia' ORDER BY id DESC LIMIT 3";
        ...
        ?>
            <div class="bg-white p-6 border border-[#8b5e34] rounded-lg shadow-md">
                <div class="w-full h-56 flex items-center justify-center bg-white">
                    <img src="uploads/<?= htmlspecialchars($h['gambar']); ?>" alt="<?= htmlspecialchars($h['nama']); ?>" class="w-full h-full object-cover rounded-md">
                </div>
                <h3 class="text-xl font-bold text-[#8b5e34] mt-3"><?= htmlspecialchars($h['nama']); ?></h3>
                <p class="mt-2 text-gray-600 text-sm"><?= htmlspecialchars($h['deskripsi']); ?></p>
                <div class="flex justify-center space-x-4 mt-4">
                    ...
                    <a href="form_adopsi.php?hewan_id=<?= $h['id']; ?>" class="bg-[#8b5e34] text-white px-4 py-2 rounded-lg shadow-md hover:bg-[#6b3f21]">Adopsi</a>
                </div>
            </div>
        <?php 
            endwhile;
        else:
            echo '<p class="text-gray-500 col-span-3">Belum ada hewan tersedia saat ini.</p>';
        endif;
        ?>
    </div>
